...
PlasmoidItem {
    ...
    property string timezoneName: Plasmoid.configuration.customTimezone !== "Local" ? Plasmoid.configuration.customTimezone : "Local"
    property string displayCity: Plasmoid.configuration.displayCity 
    
    readonly property string currentTime: {
        if (timezoneName === "Local") {
            return Qt.locale().toString(dataSource.data["Local"]["DateTime"], Qt.locale().timeFormat(Locale.LongFormat));
        } else {
            return Qt.locale().toString(dataSource.data[timezoneName]["DateTime"], Qt.locale().timeFormat(Locale.LongFormat));
        }
    }
    
    readonly property string currentDate: {
        if (timezoneName === "Local") {
            return Qt.locale().toString(dataSource.data["Local"]["DateTime"], Qt.locale().dateFormat(Locale.LongFormat).replace(/(^dddd.?\s)|(,?\sdddd$)/, ""));
        } else {
            return Qt.locale().toString(dataSource.data[timezoneName]["DateTime"], Qt.locale().dateFormat(Locale.LongFormat).replace(/(^dddd.?\s)|(,?\sdddd$)/, ""));
        }
    }
    ...
    toolTipMainText: displayCity || timezoneName
    toolTipSubText: `${currentTime}\n${currentDate}`


    function dateTimeChanged() {
        var source = timezoneName;
        var currentTZOffset = dataSource.data[source]["Offset"] / 60;
    ...
    P5Support.DataSource {
        ...
        onDataChanged: {
            var source = timezoneName;
            if (data[source]) {
                var date = new Date(data[source]["DateTime"]);
                hours = date.getHours();
    ...
    compactRepresentation: MouseArea {
        ...
        KSvg.FrameSvgItem {
            ...
            PlasmaComponents.Label {
                ...
                text: displayCity || dataSource.data[timezoneName]["Timezone"]
    ...
    fullRepresentation: PlasmaCalendar.MonthView {
        ...
        today: dataSource.data[timezoneName]["DateTime"]
    }
